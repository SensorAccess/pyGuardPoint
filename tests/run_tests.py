#!/usr/bin/env python3
"""
Integration test runner for pyGuardPoint (synchronous API).
...
from pyGuardPoint_Build.pyGuardPoint import (
    GuardPoint, Cardholder, EventOrder, GuardPointError, GuardPointUnauthorized
)
...
def main():
    ...
    cardholders = r.run("Get cardholders (limit 5)", gp.get_card_holders, limit=5)
    r.check("Get cardholders (limit 5)", cardholders is not None,
            f"{len(cardholders)} returned" if cardholders else "none")

    count = r.run("Get cardholder count", gp.get_cardholder_count)
    r.check("Get cardholder count", count is not None and count >= 0, f"total={count}")

    first = cardholders[0] if cardholders else None
    if first:
        ch = r.run("Get cardholder by UID", gp.get_card_holder, uid=first.uid)
    ...
    results = r.run("Search cardholders", gp.get_card_holders, search_terms="test", limit=5)
    r.check("Search cardholders", results is not None,
            f"{len(results)} matches" if results else "none")

    types = r.run("Get cardholder types", gp.get_cardholder_types)
    ...
    ch_new = Cardholder()
    ch_new.firstName   = f"RunTest_{test_id}"
    ch_new.lastName    = "auto"
    ch_new.pinCode     = "9999"
    ch_new.description = "Created by run_tests.py"

    created = r.run("Create cardholder", gp.new_card_holder, ch_new)
    if r.check("Create cardholder", created and created.uid,
               f"UID {created.uid[:8]}..." if created and created.uid else ""):

        created.description = "Updated by run_tests.py"
        created.pinCode = "1234"
        updated = r.run("Update cardholder", gp.update_card_holder, created)
        if r.check("Update cardholder", updated is not None):
            verified = r.run("Verify update", gp.get_card_holder, uid=created.uid)
            r.check("Verify update",
                    verified and verified.description == "Updated by run_tests.py",
                    verified.description if verified else "")

        deleted = r.run("Delete cardholder", gp.delete_card_holder, created)
        r.check("Delete cardholder", deleted, f"deleted {ch_new.firstName}")

    # ------------------------------------------------------------------ #
    # 3b. CARDHOLDER WITH fromDateValid (naive datetime → UTC)
    # ------------------------------------------------------------------ #
    print_section("3b. CARDHOLDER WITH fromDateValid")

    test_id2 = int(time.time()) % 1000000
    ch_dated = Cardholder()
    ch_dated.firstName    = f"DateTest_{test_id2}"
    ch_dated.lastName     = "auto"
    ch_dated.fromDateValid = datetime.now()  # naive datetime — no tzinfo

    created_dated = r.run("Create cardholder with fromDateValid", gp.new_card_holder, ch_dated)
    if r.check("Create cardholder with fromDateValid",
               created_dated and created_dated.uid,
               f"UID {created_dated.uid[:8]}..." if created_dated and created_dated.uid else ""):

        verified_dated = r.run("Verify fromDateValid persisted", gp.get_card_holder,
                               uid=created_dated.uid)
        r.check("Verify fromDateValid persisted",
                verified_dated and verified_dated.fromDateValid is not None,
                str(verified_dated.fromDateValid) if verified_dated else "")

        deleted_dated = r.run("Delete dated cardholder", gp.delete_card_holder, created_dated)
        r.check("Delete dated cardholder", deleted_dated, ch_dated.firstName)
    ...
    cards = r.run("Get cards (limit 5)", gp.get_cards, limit=5)
    r.check("Get cards (limit 5)", cards is not None,
            f"{len(cards)} returned" if cards else "none")

    card_count = r.run("Get card count", gp.get_cards, count=True)
    ...
    readers = r.run("Get readers", gp.get_readers)
    r.check("Get readers", readers is not None,
            f"{len(readers)} readers" if readers else "none")

    controllers = r.run("Get controllers", gp.get_controllers)
    r.check("Get controllers", controllers is not None,
            f"{len(controllers)} controllers" if controllers else "none")

    inputs = r.run("Get inputs", gp.get_inputs)
    r.check("Get inputs", inputs is not None,
            f"{len(inputs)} inputs" if inputs else "none")

    relays = r.run("Get relays", gp.get_relays)
    ...
    ev_count = r.run("Get access event count", gp.get_access_events_count)
    r.check("Get access event count", ev_count is not None and ev_count >= 0,
            f"total={ev_count}")

    events = r.run("Get access events (limit 5)", gp.get_access_events, limit=5)
    r.check("Get access events (limit 5)", events is not None,
            f"{len(events)} events" if events else "none")

    events_desc = r.run("Get events ordered DESC", gp.get_access_events,
                        limit=5, orderby=EventOrder.DATETIME_DESC)
    r.check("Get events ordered DESC", events_desc is not None,
            f"{len(events_desc)} events" if events_desc else "none")

    alarm_ev_count = r.run("Get alarm event count", gp.get_alarm_events_count)
    r.check("Get alarm event count",
            alarm_ev_count is not None and alarm_ev_count >= 0,
            f"total={alarm_ev_count}")

    audit_ev_count = r.run("Get audit event count", gp.get_audit_events_count)
    r.check("Get audit event count",
            audit_ev_count is not None and audit_ev_count >= 0,
            f"total={audit_ev_count}")

    comm_ev_count = r.run("Get comm event count", gp.get_comm_events_count)
    r.check("Get comm event count",
            comm_ev_count is not None and comm_ev_count >= 0,
            f"total={comm_ev_count}")

    general_ev_count = r.run("Get general event count", gp.get_general_events_count)
    r.check("Get general event count",
            general_ev_count is not None and general_ev_count >= 0,
            f"total={general_ev_count}")

    technical_ev_count = r.run("Get technical event count", gp.get_technical_events_count)
    r.check("Get technical event count",
            technical_ev_count is not None and technical_ev_count >= 0,
            f"total={technical_ev_count}")

    user_manual_ev_count = r.run("Get user manual event count", gp.get_user_manual_events_count)
    r.check("Get user manual event count",
            user_manual_ev_count is not None and user_manual_ev_count >= 0,
            f"total={user_manual_ev_count}")

    extended_union_events = r.run("Get extended union events (limit 5)", gp.get_extended_union_events, limit=5)
    r.check("Get extended union events (limit 5)", extended_union_events is not None,
            f"{len(extended_union_events)} events" if extended_union_events else "none")
    ...
    weekly_programs = r.run("Get weekly programs", gp.get_weekly_programs)
    r.check("Get weekly programs", weekly_programs is not None,
            f"{len(weekly_programs)} programs" if weekly_programs else "none")

    if weekly_programs:
        wp = r.run("Get weekly program by UID", gp.get_weekly_program,
                   weekly_programs[0].uid)
    ...
    infos = r.run("Get system infos", gp.get_infos)
    r.check("Get system infos", infos is not None,
            f"{len(infos)} entries" if infos else "none")

    sigr = r.run("Check SignalR enabled", gp.is_sigr_enabled)
    r.check("Check SignalR enabled", sigr is not None, f"enabled={sigr}")

    manual_events = r.run("Get manual events", gp.get_manual_events)
