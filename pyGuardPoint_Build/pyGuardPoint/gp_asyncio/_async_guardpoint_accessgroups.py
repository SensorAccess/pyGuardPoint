...
class AccessGroupsAPI:
    ...
    async def get_access_groups(self, offset: int = 0, limit: int = 500, **ag_kwargs):
        access_groups = []
        ...
        if limit > 50:
            ...
            while len(access_groups) == offset:
                ...
                if batch_limit > 0:
                    access_groups.extend(await self.get_access_groups(offset=offset + i_offset, limit=batch_limit, **ag_kwargs))
                if (offset + batch_limit) >= limit:
                    break
