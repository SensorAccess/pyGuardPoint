...
class CardholdersAPI:
    ...
    async def _count_areas(self, areas, search_terms, filter_expired, cardholder_type_name,
        ...
        try:
            return await self._get_card_holders(offset=0, limit=1, search_terms=search_terms, areas=areas,
                                                filter_expired=filter_expired,
                                                cardholder_type_name=cardholder_type_name,
            ...
            if not (isinstance(areas, list) and len(areas) > 1) or not _is_node_count_error(e):
                raise
            mid = len(areas) // 2
            left = await self._count_areas(areas[:mid], search_terms, filter_expired, cardholder_type_name,
                                           earliest_last_pass, earliest_last_pass_include_null,
                                           select_ignore_list, select_include_list, cardholder_orderby,
                                           **cardholder_kwargs)
            right = await self._count_areas(areas[mid:], search_terms, filter_expired, cardholder_type_name,
                                            earliest_last_pass, earliest_last_pass_include_null,
                                            select_ignore_list, select_include_list, cardholder_orderby,
                                            **cardholder_kwargs)
            return (left or 0) + (right or 0)

    async def _fetch_areas_all(self, areas, search_terms, filter_expired, cardholder_type_name,
                               earliest_last_pass, earliest_last_pass_include_null,
                               select_ignore_list, select_include_list, cardholder_orderby,
                               **cardholder_kwargs):
        ...
        try:
            ...
            if not (isinstance(areas, list) and len(areas) > 1) or not _is_node_count_error(e):
                raise
            mid = len(areas) // 2
            left = await self._fetch_areas_all(areas[:mid], search_terms, filter_expired, cardholder_type_name,
                                               earliest_last_pass, earliest_last_pass_include_null,
                                               select_ignore_list, select_include_list, cardholder_orderby,
                                               **cardholder_kwargs)
            right = await self._fetch_areas_all(areas[mid:], search_terms, filter_expired, cardholder_type_name,
                                                earliest_last_pass, earliest_last_pass_include_null,
                                                select_ignore_list, select_include_list, cardholder_orderby,
                                                **cardholder_kwargs)
            return left + right
