...
class CardholdersAPI:
    ...
    async def get_card_holders(self, offset: int = 0, limit: int = 10, search_terms: str = None,
        ...
        if limit > 50 and count is False:
            ...
            while len(card_holders) == offset:
                ...
                if batch_limit > 0:
                    card_holders.extend(
                        await self._split_get_card_holders_query(offset=offset + i_offset, limit=batch_limit,
            ...
            card_holders = await self._split_get_card_holders_query(offset=offset, limit=limit,
                                                                    search_terms=search_terms,
                                                                    search_match_all=search_match_all,
                                                                    areas=areas,
                                                                    filter_expired=filter_expired,
    ...
    async def _split_get_card_holders_query(self, offset: int = 0, limit: int = 10, search_terms: str = None,
        ...
        if count:
            return await self._count_areas(areas, search_terms=search_terms,
                                           search_match_all=search_match_all,
                                           filter_expired=filter_expired,
                                           cardholder_type_name=cardholder_type_name,
        ...
        combined = await self._fetch_areas_all(areas, search_terms=search_terms,
                                               search_match_all=search_match_all,
                                               filter_expired=filter_expired,
                                               cardholder_type_name=cardholder_type_name,
                                               earliest_last_pass=earliest_last_pass,
    ...
    async def _count_areas(self, areas, search_terms, search_match_all, filter_expired, cardholder_type_name,
        ...
        try:
            return await self._get_card_holders(offset=0, limit=1, search_terms=search_terms,
            ...
            if not (isinstance(areas, list) and len(areas) > 1) or not _is_node_count_error(e):
                raise
            mid = len(areas) // 2
            left = await self._count_areas(areas[:mid], search_terms, search_match_all, filter_expired, cardholder_type_name,
                                           earliest_last_pass, earliest_last_pass_include_null,
                                           select_ignore_list, select_include_list, cardholder_orderby,
                                           **cardholder_kwargs)
            right = await self._count_areas(areas[mid:], search_terms, search_match_all, filter_expired, cardholder_type_name,
                                            earliest_last_pass, earliest_last_pass_include_null,
                                            select_ignore_list, select_include_list, cardholder_orderby,
                                            **cardholder_kwargs)
            return (left or 0) + (right or 0)

    async def _fetch_areas_all(self, areas, search_terms, search_match_all, filter_expired, cardholder_type_name,
                               earliest_last_pass, earliest_last_pass_include_null,
                               select_ignore_list, select_include_list, cardholder_orderby,
                               **cardholder_kwargs):
        ...
        try:
            ...
            page_offset = 0
            while True:
                batch = await self._get_card_holders(offset=page_offset, limit=self._MAX_PAGE_SIZE,
            ...
            if not (isinstance(areas, list) and len(areas) > 1) or not _is_node_count_error(e):
                raise
            mid = len(areas) // 2
            left = await self._fetch_areas_all(areas[:mid], search_terms, search_match_all, filter_expired, cardholder_type_name,
                                               earliest_last_pass, earliest_last_pass_include_null,
                                               select_ignore_list, select_include_list, cardholder_orderby,
                                               **cardholder_kwargs)
            right = await self._fetch_areas_all(areas[mid:], search_terms, search_match_all, filter_expired, cardholder_type_name,
                                                earliest_last_pass, earliest_last_pass_include_null,
                                                select_ignore_list, select_include_list, cardholder_orderby,
                                                **cardholder_kwargs)
            return left + right

    async def _get_card_holders(self, offset: int = 0, limit: int = 10, search_terms: str = None,
                                search_match_all: bool = False, areas: list = None,
                                filter_expired: bool = False, cardholder_type_name: str = None,
                                count: bool = False, earliest_last_pass: datetime = None,
                                earliest_last_pass_include_null: bool = True,
