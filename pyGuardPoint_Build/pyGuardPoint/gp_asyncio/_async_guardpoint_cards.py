...
class CardsAPI:
    async def get_cards(self, count=False, offset: int = 0, limit: int = 500, **card_kwargs):
        url = "/odata/API_Cards"
        headers = {
        ...
        if limit > 50:
            ...
            while len(cards) == offset:
                ...
                if batch_limit > 0:
                    cards.extend(await self.get_cards(offset=offset + i_offset, limit=batch_limit, **card_kwargs))
                if (offset + batch_limit) >= limit:
                    break
    ...
    async def new_card(self, card: Card):
        ...
        if code == 201:  # HTTP CREATED
            ...
            if "errorMessages" in json_body:
                ...
                raise GuardPointError(f"Failed to create Card, check parameters ({str(code)})")
