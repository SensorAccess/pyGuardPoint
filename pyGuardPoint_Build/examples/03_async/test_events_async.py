...
async def main():
    ...
    try:
        access_events = await gp.get_access_events(limit=1, orderby=EventOrder.LOG_ID_ASC)
        for access_event in access_events:
            pprint(access_event)
            print(f"\n\n")

        alarm_events = await gp.get_alarm_events(limit=1)
        for alarm_event in alarm_events:
            pprint(alarm_event)
            print(f"\n\n")

        audit_events = await gp.get_audit_events(limit=1)
